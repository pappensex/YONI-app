export default async function handler(req, res) {
  ...
  try{
    ...
    const name=(body.name||"").slice(0,120), email=(body.email||"").slice(0,200);
    const subject=(body.subject||`Neue Nachricht von ${name||"Kontakt"}`).slice(0,200);
    ...
    const r=await fetch("https://api.resend.com/emails",{method:"POST",
      ...
      body:JSON.stringify({from,to:[to],subject,html,headers:{"Reply-To":email}})});
    const data=await r.json().catch(()=>({})); if(!r.ok) return res.status(502).json({error:"Resend failed",details:data});
    return res.status(200).json({ok:true,id:data.id});
  }catch(err){ return res.status(500).json({error:"Unexpected error",details:String(err)}); }
}
