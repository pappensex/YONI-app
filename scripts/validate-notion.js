...
function validateNotionTemplate(filePath) {
  ...
  const requiredFields = ['title', 'version', 'created', 'description', 'pillars', 'workflow'];
  ...
  data.pillars.forEach((pillar, index) => {
    const missingPillarFields = pillarRequiredFields.filter(field => !(field in pillar));
    if (missingPillarFields.length > 0) {
      console.error(`❌ Error: Pillar ${index + 1} missing fields: ${missingPillarFields.join(', ')}`);
      process.exit(1);
    }
    ...
    pillar.tasks.forEach((task, taskIndex) => {
      const missingTaskFields = taskRequiredFields.filter(field => !(field in task));
      if (missingTaskFields.length > 0) {
        console.error(`❌ Error: Task ${taskIndex + 1} in pillar "${pillar.name}" missing fields: ${missingTaskFields.join(', ')}`);
        process.exit(1);
      }

      // Validate task ID format
      if (!task.id.startsWith(pillar.name + '-')) {
        console.error(`❌ Error: Task ID "${task.id}" should start with "${pillar.name}-"`);
        process.exit(1);
      }
    });

    console.log(`✓ Pillar "${pillar.name}" has ${pillar.tasks.length} valid task(s)`);
  });
  ...
  data.pillars.forEach(pillar => {
    pillar.tasks.forEach(task => {
      if (!validStatuses.includes(task.status)) {
        console.error(`❌ Error: Task "${task.id}" has invalid status "${task.status}". Valid statuses: ${validStatuses.join(', ')}`);
        process.exit(1);
      }

      if (!validPriorities.includes(task.priority)) {
        console.error(`❌ Error: Task "${task.id}" has invalid priority "${task.priority}". Valid priorities: ${validPriorities.join(', ')}`);
        process.exit(1);
      }
    });
  });
  ...
  // Summary
  const totalTasks = data.pillars.reduce((sum, pillar) => sum + pillar.tasks.length, 0);
  console.log('\n📊 Validation Summary:');
